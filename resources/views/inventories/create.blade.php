                                      ...
                                      <div class="card-box" style="box-shadow: 0 25px 35px 0 lightgrey">
                                        ...
                                        <div class="form-row">
                                            ...
                                            <div class="form-group col-md-2">
                                               ...
                                               <option {{ old('trans_type') == 'Direct Purchase' ? 'selected' : '' }}
                                                   value="Direct Purchase">
                                                   Direct Purchase</option>
                                                   <option {{ old('trans_type') == 'Stock Purchase' ? 'selected' : '' }}
                                            ...
                                            <div class="form-group col-md-2">
                                                ...
                                                <input name="invoice_number" type="text"  onblur="duplicateInvoiceNumber(this)" value="{{ old('invoice_number') }}" required  class="form-control date-pick" placeholder="Invoice Reference">
                                            </div>

                                            <div class="form-group col-md-2">
                                                
                                                <label for="inputCity" class="col-form-label">GRN Number</label>
                                                <input name="request_number" value="{{ $grn_number }}" type="text" readonly class="form-control date-pick"
                                                placeholder={{ $grn_number }}>
                                            </div>
                                        </div>
                                        <div class="form-row">
                                            <div class="form-group col-md-2">
                                                ...
                                                <option {{ old('billing_currency') == 'Dollar' ? 'selected' : '' }}
                                                    value="Dollar">
                                                    Dollar</option>
                                            </select>
                                            </div>
                                            ...
                                            <div class="form-group col-md-3">
                                                ...
                                                <input type="text" name="exchange_rate" value="{{ old('exchange_rate') }}" min="0" step="0.00001" class="form-control" required placeholder="Exchange Rate">
                                                                       </div>
                                           
                                            <div class="form-group col-md-2">
                                                <label for="inputCity" class="col-form-label">Waybill Reference:</label>
                                                <input type="text" name="waybill" id="waybill"  onblur="duplicateWaybill(this)" value="{{ old('waybill') }}" required class="form-control" placeholder="Waybill Reference">
                                                                       </div>

                                            <div class="form-group col-md-2">
                                                <label for="inputCity" class="col-form-label"> PO Reference</label>
                                                <input name="po_number" type="text" value="{{ old('po_number') }}"  onblur="duplicatePoNumber(this)" required class="form-control date-pick" placeholder="PO Reference">
                                            </div>
                                            

                                            <div class="form-group col-md-2">
                                                <label for="inputCity" class="col-form-label"> Date</label>
                                                <input name="date" value="{{$date}}"  type="text" readonly class="form-control date-pick"
                                                placeholder={{ $date }}>
                                            </div>
              ...
              @section('scripts')
                 ...
                 function dynamic_field(number)
                 {
                  ...
                  html = '<tr>';
                        ...
                        html += '<td><select required name="item_id[]" id="products" class="form-control pid livesearch9 select2"><option value="" readonly>Chose Item</option>{{!! \App\Http\Controllers\InventoryController::getItem() !!}</select></td>';
                        ...
                        html += '<td><select required name="location_id[]"  class="form-control required  selectedjquery select2"><option value="" readonly>Chose Location</option>{{!! \App\Http\Controllers\InventoryController::fetch_locations() !!}</select></td>';
            
                      
                         html += '<td style="display:none;"> <select name="products[]" id="products" class="form-control"><option value="" readonly>Chose Part</option>{{!! \App\Http\Controllers\OrderController::fetch_products() !!}</select></td>';
...
$("select.select2").select2();
                ...
                $('tbody').delegate('.pid','change',function(){
                   ...
                   $.ajax({
                    url : "{{route('fetch_single_product')}}",
                    method:"post",
...
<script>
   ...
   {{-- <script>
   function duplicateWaybill(element){
        var waybill = $(element).val();
        $.ajax({
            type: "POST",
            url: '{{url('check_waybill')}}',
            data: {waybill:waybill,
                _token: $('meta[name="csrf-token"]').attr('content') },
            dataType: "json",
            success: function(res) {
                if(res.exists){
                    alert('waybill already exists');
                }else{
     ...
     {{-- <script>
        function duplicatePoNumber(element){
             var po_number = $(element).val();
             $.ajax({
                 type: "POST",
                 url: '{{url('check_po_number')}}',
                 data: {po_number:po_number,
                     _token: $('meta[name="csrf-token"]').attr('content') },
                 dataType: "json",
                 success: function(res) {
                     if(res.exists){
                         alert('PoNumber already exists');
                     }else{
                         // alert('does not exist');
                     }
                 },
