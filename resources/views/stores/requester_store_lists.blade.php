            ...
            <div class="card-body">
                <table id="example1" class="table table-bordered table-striped">
                    ...
                    <tbody>
                        ...
                        @foreach ($requester_store_lists as $order)
                            <tr>
                                <td>{{ $order->id }}</td>
                                ...
                                @if ($order->approval_status == 'Denied' || $order->approval_status == 'Approved')
                                    <td>N/A </td>
                                @elseif($order->approval_status === null)
                                    <td><a class="btn btn-secondary"
                                            href="{{ route('stores.requester_edit', $order->id) }}">Edit</a></td>
                                @endif

                                <td><a href="{{ route('sorders.requester_store_list_view', $order->id) }}"
                                ...
                                @else
                                    <td>
                                        <form action="{{ route('sorders.destroy', $order->id) }}" method="post">
                                            @csrf
                                            @method('DELETE')
